...
function TransactionRow({ tx, selected, onToggle }: RowProps) {
  return (
    <tr className={selected ? 'bg-white dark:bg-zinc-700' : 'bg-gray-50 dark:bg-zinc-800/50 opacity-50'}>
      <td className="px-3 py-2">
        <input type="checkbox" checked={selected} onChange={onToggle} className="rounded" />
      </td>
      <td className="px-3 py-2 text-xs text-gray-500 dark:text-zinc-400 tabular-nums">{tx.date}</td>
      <td className="px-3 py-2 text-sm text-gray-900 dark:text-zinc-100">{tx.description}</td>
      <td className="px-3 py-2 text-xs text-gray-500 dark:text-zinc-400">{tx.category}</td>
...
export function Import() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-zinc-100">Import Statement</h1>
      ...
      {/* How it works */}
      <div className="bg-indigo-50 border border-indigo-100 text-indigo-800 dark:bg-indigo-950/40 dark:border-indigo-800 dark:text-indigo-300 rounded-xl p-4 text-xs space-y-1">
        <p className="font-medium text-sm">What the Files API adds</p>
        <ol className="list-decimal list-inside space-y-0.5 text-indigo-700 dark:text-indigo-400">
      ...
      {!preview && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium text-gray-700 dark:text-zinc-300">CSV content</label>
            <button
          ...
          <textarea
            ...
            className="w-full bg-white border border-gray-300 text-gray-900 placeholder-gray-400 font-mono dark:bg-zinc-600 dark:border-zinc-500 dark:text-zinc-100 dark:placeholder-zinc-400 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
          />
          <button
      ...
      {extractError && <p className="text-red-500 dark:text-red-400 text-sm">{extractError}</p>}

      {extracting && (
        <div className="bg-white border border-gray-200 dark:bg-zinc-700 dark:border-zinc-600 rounded-xl p-5">
          <p className="text-sm text-gray-400 dark:text-zinc-500 animate-pulse">Uploading to Files API and extracting…</p>
        </div>
      )}

      {/* Import success */}
      {importedCount > 0 && (
        <div className="bg-emerald-50 border border-emerald-200 dark:bg-emerald-950/40 dark:border-emerald-800 rounded-xl p-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              {preview.notes && (
                <p className="text-xs text-gray-500 dark:text-zinc-400 mt-0.5 italic">{preview.notes}</p>
              )}
            </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 dark:bg-zinc-700/50 border-b border-gray-200 dark:border-zinc-600">
                <tr>
                  ...
                  <th className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wide">Date</th>
                  <th className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wide">Description</th>
                  <th className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wide">Category</th>
                  <th className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wide text-right">Amount</th>
                </tr>
              </thead>
