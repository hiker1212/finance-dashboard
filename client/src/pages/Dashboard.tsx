        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={label} className={`rounded-xl p-5 ${bg}`}>
                <p className="text-xs font-medium text-gray-500 dark:text-zinc-500 uppercase tracking-wide mb-1">{label}</p>
          ...
          <div className="bg-white border border-gray-200 dark:bg-zinc-700 dark:border-zinc-600 rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-zinc-300">Recent transactions</h2>
              <Link to="/transactions" className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">View all →</Link>
            </div>
            {recent.length === 0 ? (
                ...
                {recent.map(t => (
                  <div key={t.id} className="flex items-center gap-3 py-2.5">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 dark:text-zinc-100 truncate">{t.description}</p>
                      <p className="text-xs text-gray-500 dark:text-zinc-400">{t.date}</p>
                    </div>
                    {t.category_name && (
          ...
          <div className="bg-white border border-gray-200 dark:bg-zinc-700 dark:border-zinc-600 rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-zinc-300">Monthly Score</h2>
              <button
            ...
            {scoreError && <p className="text-red-500 dark:text-red-400 text-sm">{scoreError}</p>}

            {scoreLoading && (
              <p className="text-gray-400 dark:text-zinc-500 text-sm animate-pulse">Evaluating your financial health…</p>
            )}
              ...
              return (
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-lg font-bold text-gray-900 dark:text-zinc-100 tabular-nums">{score.overall_score}/10</span>
                      ...
                      <p className="text-sm text-gray-600 dark:text-zinc-400 italic">"{score.verdict}"</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <ul className="space-y-1.5">
                        {score.strengths.map((s, i) => (
                          <li key={i} className="flex gap-2 text-sm text-gray-700 dark:text-zinc-300">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wide mb-2">Warnings</p>
                      <ul className="space-y-1.5">
          ...
          <div className="bg-white border border-gray-200 dark:bg-zinc-700 dark:border-zinc-600 rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-zinc-300">AI Spending Insights</h2>
              <button
            ...
            {insightsError && <p className="text-red-500 dark:text-red-400 text-sm">{insightsError}</p>}
            {insightsLoading && (
              <p className="text-gray-400 dark:text-zinc-500 text-sm animate-pulse">Analyzing your spending data…</p>
            )}
            {!insightsLoading && !insightsError && insights && (
              <p className="text-sm text-gray-700 dark:text-zinc-300 leading-relaxed whitespace-pre-wrap">{insights}</p>
            )}
            {!insightsLoading && !insightsError && !insights && (
              <p className="text-gray-400 dark:text-zinc-500 text-sm text-center py-4">
